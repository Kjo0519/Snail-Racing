...
function restart() {
    ...
    // 코인 표시
    document.getElementById("balance").innerText = `코인: ${balance.toLocaleString()}`;
...
function placeBetSnail(snailId) {
    if (betPlaced) {
        alert("이미 배팅이 완료되었습니다. 레이스를 시작하세요!");
        return;
    }
...
function startSnailRace() {
    ...
    snails.forEach(snail => {
        snail.style.left = '0vw'; // 초기 위치를 vw 단위로 설정
    });
    ...
    const raceDistance = 55; // 화면 너비의
    let speeds = Array.from({ length: 3 }, () => Math.random() * 0.5 + 0.2); // 속도 조정
    let accelerations = Array.from({ length: 3 }, () => Math.random() * 0.05 + 0.01); // 가속도 조정

    let finished = false;

    const interval = setInterval(() => {
        snails.forEach((snail, index) => {
            let currentLeft = parseFloat(snail.style.left); // vw 단위 값 가져오기

            if (Math.random() < 0.1) {
                speeds[index] = Math.random() * 0.5 + 0.2;
                accelerations[index] = Math.random() * 0.05 + 0.01;
            }

            if (currentLeft < raceDistance) {
                speeds[index] += accelerations[index];
                snail.style.left = currentLeft + speeds[index] + 'vw'; // vw 단위로 이동
            } else if (!finished) {
                finished = true;
                ...
                if (winner === selectedSnail) {
                    ...
                    alert(`축하합니다! ${winner}가 승리했습니다! ${betAmountSnail.toLocaleString() * 3}의 코인을 획득했습니다.`);
                    if (balance >= success_coin) {
                        success();
                    }
                } else {
                    balance -= betAmountSnail; // 패배 시 배팅 금액 손실
                    const event_type = "게임 패배";
                    const amount = betAmountSnail;
                    const comment = "";
                    // log(username, event_type, comment, winner)
                    alert(`안타깝네요! ${winner}가 승리했습니다. 코인 ${betAmountSnail.toLocaleString()}개를 잃었습니다.`);
